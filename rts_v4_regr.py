...
brent = pull_intraday_data("data\\.csv", dates)
gold  = pull_intraday_data("data\\.csv", dates)

data = np.append(data, usdrub.reshape((sz, 1)), axis = 1)
data = np.append(data, sandp .reshape((sz, 1)), axis = 1)
data = np.append(data, brent .reshape((sz, 1)), axis = 1)
data = np.append(data, gold  .reshape((sz, 1)), axis = 1)
